...
def encode(content: bytes,scale=None,ratio=None,requested_width=None,requested_height=None,reserve_unused=None,transparent_unused=False,showProgress=None):
    ...
    height = 19
    if requested_height!=None:
        height = max(requested_height,10)
    ...
    elif requested_width!=None:
        width = max(requested_width,11)
        ...
        if ratio!=None:
            ...
            rawwidth*=math.sqrt(ratiof)
            rawheight/=math.sqrt(ratiof)
        if reserve_unused!=None:
            rpx=reserve_unused/100
            mult=1+rpx
    ...
    def processPixel(x,y):
        nonlocal octal,idx,eof # i hate this keyword so much
        if len(octal)==0:
            if idx==len(content):
                octal="777"
                ...
                if showProgress!=None:
                    showProgress(idx,len(content))
            idx+=1
        if idx>len(content)+1 and transparent_unused:
...
if __name__ == '__main__':
    ...
    def showProgress(idx,length):
        if length>10000 and idx%2048==0: # if file is more than 10KB
            print(f"\r\x1b[2K{round(idx/length*100)}% ({idx}/{length})",end="")

    img = encode(content,args.scale,args.ratio,args.width,args.height,args.reserve_unused,args.transparent_unused,showProgress)
